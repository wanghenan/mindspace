...
### Task 3.2: AI对话服务层（核心）
- [ ] 集成AI API（DashScope/OpenAI）
- [ ] 实现核心系统提示词注入
- [ ] 实现三阶段对话逻辑（Holding → Exploring → Action）
- [ ] 实现危机检测逻辑（Panic Attack、Self-Harm）
- [ ] 实现上下文管理（对话历史）

### Task 3.3: 对话状态管理与持久化
- [ ] 设计对话数据结构
- [ ] 实现Zustand store管理对话状态
- [ ] 实现localStorage持久化
- [ ] 实现对话历史加载
- [ ] 实现情绪标签自动生成（后台）
...
2. **状态管理**
   - 使用React Router的location state传递数据
   - 类型安全的TypeScript定义
...
- [ ] SOS卡片页的呼吸动画需要优化（目前使用简单的scale动画）
- [ ] 需要添加错误处理和加载状态
- [ ] 需要添加路由守卫（防止直接访问某些页面）
